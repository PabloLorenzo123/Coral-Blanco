...
def SearchRoom(request):
    ...
    r_adults = request.GET.get('adults')
    r_children = request.GET.get('children')
    r_check_in_date = request.GET.get('start_date')
    r_check_out_date = request.GET.get('end_date')

    # check if he has a cart.
    if ReservationCart.objects.filter(user=request.user).exits():
    ...
    ReservationCart.objects.create(
            user=request.user,
            checkin_date = r_check_in_date,
            checkout_date = r_check_out_date,
    )
    ...
    for room_type in RoomType.objects.all():
        if room_type.max_adults <= r_adults and room_type.max_children <= r_children:
            ...
            available_rooms = Room.objects.filter(
                ...
                id_in = RoomReserved.objects.filter(
            ...
            available_rooms[room_type] = [False, False]
    
    print('available rooms = ' + str(available_rooms))
